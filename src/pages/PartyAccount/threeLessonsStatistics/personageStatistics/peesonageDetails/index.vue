<template>
  <div class="org">
    ...
    <div class="searchtop">
      ...
      <el-select v-model="org" size="small" style="width:200px;margin-right:20px" placeholder="请选择会议分类">
        <el-option v-for="(item, index) in options" :key="index" :value="item"></el-option>
      </el-select>
      <el-select v-model="searchstatus" size="small" style="width:200px;margin-right:20px" placeholder="请选择参会情况">
        <el-option label="未参会" value="0"></el-option>
        <el-option label="已参会" value="1"></el-option>
    ...
    <div class="bodyright">
      ...
      <el-table :data="tableData.list" style="width: 100%;background-color:#fff">
        ...
        <el-table-column prop="location" label="会议地点" width="300"></el-table-column>
        <el-table-column prop="conferenceDate" label="会议时间"></el-table-column>
        <el-table-column prop="status" label="参会情况">
      <template slot-scope="scope">
       <div v-if="scope.row.status==0">未参会</div>
       <div v-if="scope.row.status==1">已参会</div>
       <div v-if="scope.row.status==2">请假</div>
...
<script>
import { getConferenceType,getPersonalDataDetail } from "@/api/http";
export default {
  data() {
    return {
      total: 0,
  ...
  methods: {
   ...
   async getPersonalDataDetailvue(){
      let res = await getPersonalDataDetail(
      {
        id:this.id,
